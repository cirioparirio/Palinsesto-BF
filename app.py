...
def format_time_column(df):
    ...
    if 'ORA' in df.columns:
        # Converti solo le celle che contengono orari in formato hh:mm:ss
        mask = df['ORA'].astype(str).str.contains(':', na=False)
        
        # Crea una copia per evitare SettingWithCopyWarning
        df_copy = df.copy()
        
        # Applica la formattazione solo alle celle che contengono orari
        for i in df_copy.index:
            if mask.loc[i]:
                try:
                    # Prova a convertire in datetime e poi formattare
                    time_str = df_copy.loc[i, 'ORA']
                    if isinstance(time_str, str) and ':' in time_str:
                        # Estrai solo le ore e i minuti
                        parts = time_str.split(':')
                        if len(parts) >= 2:
                            df_copy.loc[i, 'ORA'] = f"{parts[0]}:{parts[1]}"
                except:
