...
st.markdown("""
Questa applicazione permette di visualizzare e filtrare i dati del palinsesto Betfair e dell'archivio Forebet.
...
data_type = st.sidebar.radio(
    ...
    ["Palinsesto Betfair", "Archivio Forebet"]
)

# Caricamento del file Excel per il palinsesto
if data_type == "Palinsesto Betfair":
    ...
    if st.session_state.palinsesto_loaded and st.session_state.palinsesto_df is not None:
            ...
            if len(unique_values) > 10:
                ...
                if filter_type == "Testo":
                    ...
                    if np.issubdtype(col_dtype, np.number):
                        ...
                        if len(date_min) == 2:  # Se sono state selezionate due date
        ...
        st.write(f"File caricato: {st.session_state.palinsesto_file_name}")
...
# Caricamento del file Excel per l'archivio
else:  # data_type == "Archivio Forebet"
    uploaded_file = st.sidebar.file_uploader("Carica un nuovo file Excel per l'Archivio", type=["xlsx"], key="archivio_uploader")
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            st.sidebar.success(f"File caricato con successo: {uploaded_file.name} (foglio 'Giornata Odierna')")
    ...
    if st.session_state.archivio_loaded and st.session_state.archivio_df is not None:
        ...
        # Opzioni di visualizzazione
        st.header("Dati dell'Archivio Forebet")
        
        # Mostra il nome del file caricato
        st.write(f"File caricato: {st.session_state.archivio_file_name} (foglio 'Giornata Odierna')")
        
        # Mostra il numero di righe filtrate
        st.write(f"Visualizzazione di {filtered_df.shape[0]} righe su {df.shape[0]} totali")
        
        # Aggiungi un link per scaricare i dati filtrati
        st.markdown(get_table_download_link(filtered_df, "archivio_filtrato.csv", "Scarica i dati filtrati come CSV"), unsafe_allow_html=True)
