                            ...
                            try:
                                ...
                                if len(min_time_parts) >= 2 and len(max_time_parts) >= 2:
                                    ...
                                    filtered_df = filtered_df[
                                        filtered_df[column].astype(str).apply(
                                            lambda x: self_compare_time(x, min_hour, min_minute, max_hour, max_minute)
                                        )
                                    ]
                            except:
...
def self_compare_time(time_str, min_hour, min_minute, max_hour, max_minute):
    ...
    # Se c'è un errore o il formato non corrisponde, mantieni la riga
    return True
