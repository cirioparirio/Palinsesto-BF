...
# Funzione per verificare se una colonna è numerica
def is_numeric_column(df, column):
    """
    Verifica se una colonna contiene valori numerici
    """
    try:
        # Converti la colonna in numerica e verifica se ci sono valori non-NaN
        numeric_values = pd.to_numeric(df[column], errors='coerce')
        return not numeric_values.dropna().empty
    except:
        return False
...
if data_type == "Palinsesto BF":
    ...
    if st.session_state.palinsesto_loaded and st.session_state.palinsesto_df is not None:
        ...
        # Precalcola le colonne numeriche per migliorare le prestazioni
        numeric_columns = {}
        for column in df.columns:
            numeric_columns[column] = is_numeric_column(df, column)
                    ...
                    if column == 'ORA':
                        ...
                        if min_time and max_time:
                            ...
                            try:
                                ...
                                st.warning(f"Formato ora non valido. Usa il formato hh:mm.")
                    # Gestione per colonne numeriche (incluse quelle che non sono riconosciute automaticamente come numeriche)
                    elif numeric_columns[column] or column in ['ABBINATE', '1 PUNTA', 'ABB 1 PUNTA', '2 PUNTA', 'ABB 2 PUNTA', 'PARI', 'ABB PARI', 'DISPARI', 'ABB DISPARI', 'GOAL', 'ABB GOAL', 'NOGOAL', 'ABB NOGOAL', 'OVER', 'ABB OVER', 'UNDER', 'ABB UNDER', 'GG', 'ABB GG', 'NG', 'ABB NG', '1X', 'ABB 1X', 'X2', 'ABB X2', '12', 'ABB 12', 'CASA', 'ABB CASA', 'FUORI', 'ABB FUORI', 'BANCA', 'ABB 2 BANCA']:
                        ...
                        numeric_filtered_df = pd.to_numeric(filtered_df[column], errors='coerce')
